...
def _task_similarity(a: str, b: str) -> float:
    """Return word-level Jaccard similarity in [0, 1]."""
...
class ImmuneMemoryPlugin(CambrianPlugin):
    """Plugin wrapper for the immune_memory module."""

    name = "immune_memory"
    description = "Immune memory: stores and recalls successful defense strategies against adversarial evaluation tasks."
    category = "memory"
    impact = None

    def register(self, engine: Any) -> None:
        pass  # ImmuneCortex is used standalone; no engine hooks required
