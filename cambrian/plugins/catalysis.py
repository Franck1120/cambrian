...
class CatalystSelector:
    ...
    * ``fitness_weight`` x agent.fitness
    * ``vocab_weight`` x normalised prompt vocabulary size
    * ``strategy_weight`` x normalised count of strategy keywords
...
class CatalysisPlugin(CambrianPlugin):
    """Plugin wrapper for the catalysis module."""

    name = "catalysis"
    description = "Catalytic acceleration: identifies and amplifies high-impact mutations for faster convergence."
    category = "optimization"
    impact = None

    def register(self, engine: Any) -> None:
        pass  # CatalysisEngine is used standalone; no engine hooks required
