...
* An agent with *low* fitness (below ``stress_threshold``) is identified as
  "stressed".
* The stress intensity ``s = 1 - fitness / stress_threshold`` (in [0, 1])
  determines the mutation boost.
* Mild stress  (s < ``mild_cutoff``)  -> small boost to mutation temperature.
* Moderate stress (s in [mild, severe))-> medium boost + strategy hint injection.
...
class HormesisAdapter:
    ...
    def _stress_intensity(self, fitness: float) -> float:
        """Compute stress intensity in [0, 1].  0 = barely stressed, 1 = worst."""
...
class HormesisPlugin(CambrianPlugin):
    """Plugin wrapper for the hormesis module."""

    name = "hormesis"
    description = "Hormesis adaptation: beneficial stress injections that trigger enhanced performance responses."
    category = "adaptation"
    impact = None

    def register(self, engine: Any) -> None:
        pass  # HormesisAdapter is used standalone; no engine hooks required
