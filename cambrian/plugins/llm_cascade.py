...
class LlmCascadePlugin(CambrianPlugin):
    """Plugin wrapper for the llm_cascade module."""

    name = "llm_cascade"
    description = "LLM cascade: routes tasks through a cost-tiered model cascade, using expensive models only when needed."
    category = "routing"
    impact = None

    def register(self, engine: Any) -> None:
        pass  # LLMCascade is used standalone; no engine hooks required
