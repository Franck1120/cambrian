...
class SymbioticFuser:
    ...
    def _prompt_distance(a: str, b: str) -> float:
        """Approximate normalised word-level Jaccard distance in [0, 1]."""
        if not a and not b:
            return 0.0
...
class SymbiosisPlugin(CambrianPlugin):
    """Plugin wrapper for the symbiosis module."""

    name = "symbiosis"
    description = "Symbiotic genome fusion: merges complementary genomes from different evolutionary niches."
    category = "coordination"
    impact = None

    def register(self, engine: Any) -> None:
        pass  # SymbioticFuser is used standalone; no engine hooks required
