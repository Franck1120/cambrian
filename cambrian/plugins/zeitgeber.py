...
----------
ZeitgeberClock
    Tracks the generation and computes the current phase in [0, 2pi] and
    the resulting ``exploration_factor`` in [0, 1].
...
class ZeitgeberClock:
    ...
    def exploration_factor(self) -> float:
        ...
        at phase=pi.  The amplitude parameter scales the oscillation around 0.5.
        """
        # sin in [-1, 1] -> scale to [0.5 - amplitude/2, 0.5 + amplitude/2]
...
class ZeitgeberScheduler:
    ...
    mutation_range:
        ...
        (default 0.2, so rate in [0.1, 0.5]).
    base_threshold:
...
class ZeitgeberPlugin(CambrianPlugin):
    """Plugin wrapper for the zeitgeber module."""

    name = "zeitgeber"
    description = "Zeitgeber scheduling: synchronizes evolutionary cycles with circadian-like rhythm patterns."
    category = "scheduling"
    impact = None

    def register(self, engine: Any) -> None:
        pass  # ZeitgeberScheduler is used standalone; no engine hooks required
