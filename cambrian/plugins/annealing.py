...
AnnealingSchedule
    ...
    * ``"cosine"``  — T = T_min + 0.5*(T_max-T_min)*(1+cos(pi*t/n_steps))
...
class AnnealingSchedule:
    ...
    def temperature(self, t: int) -> float:
        """Return the temperature at step *t* in [0, n_steps]."""
...
class AnnealingPlugin(CambrianPlugin):
    """Plugin wrapper for the annealing module."""

    name = "annealing"
    description = "Simulated annealing selection: controls exploration vs. exploitation through temperature scheduling."
    category = "selection"
    impact = None

    def register(self, engine: Any) -> None:
        pass  # AnnealingSelector is used standalone; no engine hooks required
