...
diversity, surprise) and emits a scalar *modulation level* in [0, 1].
...
class NeuromodulatorBase(ABC):
    ...
    def level(self, population: list[Agent], generation: int) -> float:
        """Return modulation level in [0, 1]."""
...
class NeuromodulationPlugin(CambrianPlugin):
    """Plugin wrapper for the neuromodulation module."""

    name = "neuromodulation"
    description = "Neuromodulation: dynamically adjusts agent behavior via simulated neurotransmitter states."
    category = "coordination"
    impact = None

    def register(self, engine: Any) -> None:
        pass  # NeuromodulatorBank is used standalone; no engine hooks required
